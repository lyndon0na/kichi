...
| 文件页 | 类型矢量图标、可点击排序表头（名称 / 大小 / 修改时间）、可拖拽列宽、列表 / 网格双视图、当前目录名称过滤 |
...
| 文件管理 | 目录浏览（面包屑）、排序、过滤、列表 / 网格双视图、复制 / 剪切 / 粘贴、新建 / 重命名 |
| 本地上传 | gcid 秒传 + 阿里云 OSS 分片（并发、断点续传）、目录递归、历史持久化 |
| 本地下载 | `.part` + Range 断点续传、原子改名、完整性校验、退避重试、历史持久化 |
| 离线下载 | 磁力 / 直链转存、分桶轮询（有进行中任务时加快）、分页「加载更多」、失败重试 / 删除 / 整组清空、保存位置可选 |
| 分享 | 创建与管理分享链接；转存他人分享到自己网盘 |
| 回收站 | 列出 / 还原 / 彻底删除 / 一键清空，SWR 缓存 |
| 文件预览 | `mpv` 流式播放（清晰度选择 + 外挂字幕）、系统查看器回退 |
...
- 「传输任务」页下载分栏展示任务列表（进度 / 速率 / 取消 / 打开目录），带状态筛选与批量选择、批量重试 / 移除
...
**离线下载（磁力 / HTTP）**
- 提交 magnet / 直链转存到云端
- 状态分桶展示：等待 / 下载中 / 已完成 / 失败（自动轮询，存在进行中任务时加快节拍）
