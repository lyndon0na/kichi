...
- **本地上传**
  - 文件页工具栏 / 空白右键菜单「上传文件」，或「传输任务 ▸ 上传」上传到当前网盘目录
  - 先算 gcid 秒传哈希：服务端已有相同内容时直接完成（秒传），否则走阿里云 OSS 分片上传
  - 分片签名走 OSS HMAC-SHA1（不引入 AWS SDK）；进度 / 速率、取消、失败重试
...
- 未实现：分享链接解析 / 转存（「我的分享」为占位）。
- 「回收站」页面为占位：可将文件移入回收站，但暂不支持在应用内浏览 / 还原回收站内容。
