use eframe::egui::{
    self, vec2, Align, CornerRadius, FontId, Frame, Layout, Margin, Pos2, Rect, Stroke,
};
...
impl App {
    ...
    pub(super) fn nav_item(
        ...
        let txt = painter.layout_no_wrap(label.to_string(), FontId::proportional(14.0), txt_color);
        painter.galley(
        ...
        if let Some(n) = badge {
            ...
            painter.galley(
                Pos2::new(r.center().x - g.size().x / 2.0, r.min.y + 3.0),
                g,
                badge_fg,
            );
        }
    ...
    pub(super) fn sidebar(&mut self, ui: &mut egui::Ui, th: &Theme) {
        ...
        let row_h = 40.0;
        let (brect, _) =
            ui.allocate_exact_size(vec2(ui.available_width(), row_h), egui::Sense::hover());
        let bp = ui.painter().clone();
        let logo = Rect::from_center_size(
            Pos2::new(brect.min.x + 18.0, brect.center().y),
            vec2(30.0, 30.0),
        );
        bp.rect_filled(logo, th.cr(9), th.accent);
        let ltxt = bp.layout_no_wrap("K".into(), FontId::proportional(17.0), th.on_accent);
        bp.galley(
            Pos2::new(
                logo.center().x - ltxt.size().x / 2.0,
                logo.center().y - ltxt.size().y / 2.0,
            ),
            ltxt,
            th.on_accent,
        );
        ...
        bp.galley(
            Pos2::new(brect.min.x + 42.0, brect.min.y + 24.0),
            sub,
            th.text_faint,
        );
    ...
    pub(super) fn account_card(
        ...
        let av = Rect::from_center_size(
            Pos2::new(rect.min.x + 19.0, rect.center().y),
            vec2(30.0, 30.0),
        );
        ...
        painter.galley(
            Pos2::new(
                av.center().x - g.size().x / 2.0,
                av.center().y - g.size().y / 2.0,
            ),
            g,
            th.accent,
        );
        let name_rect = Rect::from_min_max(
            Pos2::new(rect.min.x + 42.0, rect.min.y + 4.0),
            Pos2::new(rect.right() - 44.0, rect.max.y - 4.0),
        );
        painter.rect_filled(
            Rect::from_min_max(
                Pos2::new(rect.min.x + 42.0, rect.min.y + 5.0),
                Pos2::new(rect.right() - 40.0, rect.max.y - 5.0),
            ),
            th.cr(8),
            egui::Color32::TRANSPARENT,
        );
        let uname = truncate_text(
            &painter,
            username,
            name_rect.width(),
        ...
        painter.galley(
            Pos2::new(name_rect.min.x, rect.center().y - uname.size().y / 2.0),
            uname,
            th.text,
        );
    ...
    pub(super) fn quota_card(
        ...
        let (rect, _) =
            ui.allocate_exact_size(vec2(ui.available_width(), 74.0), egui::Sense::hover());
        let card = rect;
        let painter = ui.painter().clone();
        let (r, q) = match quota {
        ...
        let title =
            painter.layout_no_wrap("存储空间".into(), FontId::proportional(12.0), th.text_weak);
        painter.galley(
        ...
        if quota.is_some() {
            ...
            painter.galley(
                Pos2::new(inner.right() - gp.size().x, inner.min.y + 2.0),
                gp,
                th.text_faint,
            );
        }
        ...
        painter.rect_filled(
            bar,
            th.cr(2),
            mix(th.bg, th.text, if th.dark { 0.12 } else { 0.1 }),
        );
        if frac > 0.0 {
        ...
        let usage_txt = match q {
            Some(q) => format!(
                "已用 {} / 共 {}",
                format::fmt_bytes(q.usage),
                format::fmt_bytes(q.limit)
            ),
            None => "正在获取…".into(),
        };
