...
- 本地下载：文件页选中/右键、离线「已完成」任务一键下载；`.part`+Range 断点续传、
  ...
  下载目录记忆
- 文件预览：双击或右键「打开 / 预览」；音/视频由 `mpv` 流式播放(带签名直链请求头)，
  其他文件下载到 `~/.cache/pikpak-linux/preview` 后交给系统查看器(`xdg-open`)，
  命中缓存不重复下载，缺少 `mpv` 时回退系统播放器
...
## 六、质量
- `cargo test --workspace`：核心库 12 项测试 + GUI 5 项通过（加签 golden 向量、JSON 解析、
  token 边界、直链解析回退、文件名净化、batchMove/batchCopy 请求体、媒体类型识别）
