...
| 离线任务翻页 | 无翻页：每个 `phase` 仅取首页（最多 100 条），超出部分不展示；任务字段防御式解析，展示信息有限 |
...
离线任务 JSON 未公开、字段不稳定，因此按 `phase`（PENDING / RUNNING / COMPLETE / ERROR）分四次查询分桶展示，状态以请求结果为准。
