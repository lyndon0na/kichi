...
- 新建文件夹 / 重命名 / 移入回收站（支持多选、右键、Delete 键）；回收后本地即时隐藏，
  服务端列表同步完成后自动解除
- 移动 / 复制到其他目录（支持多选、工具栏/右键入口）；弹窗内浏览目标目录（进入/返回上级/
  面包屑跳转/刷新/分页），自动排除被选中的源文件夹避免目录移入自身
...
### 未实现 / 已知边界
- 本地上传 / 分享转存 / 文件预览 / 搜索；本地整目录下载(会跳过文件夹)
...
## 六、质量
- `cargo test --workspace`：核心库 12 项测试 + GUI 3 项通过（加签 golden 向量、JSON 解析、
  token 边界、直链解析回退、文件名净化、batchMove/batchCopy 请求体）
